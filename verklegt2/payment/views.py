...
def finalize_offer(request, offer_id, step):
    ...
    if step == "contact":
        ...
        if request.method == "POST" and form.is_valid():
            ...
            return redirect("finalize_offer", offer_id=offer.id, step=selected_payment_method)

        selected_payment_method = request.POST.get("payment_method") or request.session.get("payment_method")
        ...
        offer.status = "finalized"  # or whatever status you use
        offer.save()

        # Clean session after purchase
